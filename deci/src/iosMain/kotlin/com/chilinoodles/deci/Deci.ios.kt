...
actual class Deci private constructor(
    ...
    private fun toNativeMode(
        ...
        RoundingMode.UP ->  // Java “away from zero”
            if (isPositive())
                NSRoundingMode.NSRoundUp      // + → ceiling = away
            else
                NSRoundingMode.NSRoundDown    // – → floor   = away

        RoundingMode.DOWN ->  // Java “toward zero”
            if (isPositive())
                NSRoundingMode.NSRoundDown   // + → floor   = toward zero
            else
                NSRoundingMode.NSRoundUp     // – → ceiling = toward zero

        RoundingMode.CEILING -> NSRoundingMode.NSRoundUp    // always +∞
        RoundingMode.FLOOR -> NSRoundingMode.NSRoundDown  // always -∞
